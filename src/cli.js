...
function help() {
  console.log('options');
  console.log(
    'git connect  - connects with project tools like gitlab, bitbucket, github and trello'
  );
}
export function cli(args) {
  console.log(
    chalk.yellow(figlet.textSync('GIT TASKS', { horizontalLayout: 'full' }))
  );
...
function gitlab() {
  try {
    inquirer
      ...
      .then((option) => {
        let gitlab_url =
          'https://gitlab.com/api/v4/projects?private_token=&visibility=private&simple=true&archived=false' +
          `&search=` +
          option.source;
        requestify
          .request(gitlab_url, {
